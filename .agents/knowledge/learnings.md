...
| 2026-06-02 | CI & Favicon | **NODE CI WORKFLOW & PERFECTLY CENTERED FAVICON**: Replaced obsolete Java/Maven CI workflow (which failed due to missing `pom.xml`) with a modern Node.js React + Firebase functions CI pipeline using Node 20. Overwrote `favicon.svg` with a beautifully centered gradient graduation cap logo and added a `?v=2` cache-busting link reference to bypass aggressive browser caching. | Rewrote `.github/workflows/ci.yml`, updated `frontend/public/favicon.svg`, rebuilt, tested, and deployed to GitHub and Firebase Hosting. |
